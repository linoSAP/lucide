...
export function BottomNav() {
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-4 z-30 px-4">
      <nav className="pointer-events-auto mx-auto flex w-full max-w-md items-center justify-between rounded-full border border-white/5 bg-card/90 px-2 py-2 shadow-soft backdrop-blur-xl">
        {items.map((item) => {
              ...
              {({ isActive }) => (
                <span
                  className={cn(
                    ...
                    isActive ? "bg-white/6 text-foreground" : "text-muted-foreground hover:text-foreground",
                  )}
                >
